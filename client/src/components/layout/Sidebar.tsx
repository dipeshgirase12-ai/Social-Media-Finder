...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r dt-border bg-[var(--color-card)] pt-20 lg:flex" aria-label="Sidebar">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg border border-transparent px-3 py-2.5 text-sm transition-colors ${
                  isActive ? 'border-primary-400/20 bg-primary-400/10 font-medium text-primary-300' : 'dt-muted hover:bg-surface-overlay hover:text-[var(--color-text)]'
              }`
            }
          >
      ...
      <div className="border-t dt-border p-4 space-y-3">
        ...
        {user && (
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm dt-muted hover:text-[var(--color-text)] hover:bg-surface-overlay"
          >
